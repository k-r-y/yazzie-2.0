...
async function loadUpcoming() {
    ...
    tbody.innerHTML = events.map(b => `
        <tr>
            ...
            <td>${b.menu_name}</td>
            <td>${b.pax_count}</td>
            <td class="text-sm text-muted">${(b.event_location || '—').substring(0, 35)}${b.event_location?.length > 35 ? '…' : ''}</td>
            <td>
...
async function loadUnpaidBalances() {
    ...
    div.innerHTML = list.map(b => {
        ...
        <div style="padding:14px 20px;border-bottom:1px solid var(--border);">
            <div class="fw-700" style="font-size:14px;">${b.client_name}</div>
            <div class="text-sm text-muted">${Format.dateShort(b.event_date)} &mdash; ${b.menu_name}</div>
            ...
            <div style="display:flex; justify-content:space-between; align-items:center; margin-top:6px;">
                ...
                <a href="${BASE}/views/admin/financial.php?booking_id=${b.id}" class="btn btn-outline-primary btn-sm">Pay</a>
            </div>
        </div>`;
    }).join('');
}
